...
static int persist(void) {
    ...
    zero(all, sizeof(all));
    *(uint32_t *)all = VFS_MAGIC; *(uint32_t *)(all + 4) = VFS_VERSION;
    *(uint32_t *)(all + 8) = ++generation; *(uint32_t *)(all + 12) = VFS_FILE_MAX;
    for (i = 0; i < VFS_FILE_MAX; ++i) all[16 + i] = allocation[i];
    offset = 0;
    for (i = 0; i < VFS_FILE_MAX; ++i) {
        uint8_t *raw = (uint8_t *)&records[i];
        uint32_t j;
        for (j = 0; j < sizeof(struct vfs_record); ++j) all[16 + VFS_FILE_MAX + offset++] = raw[j];
    }
    for (i = 0; i < 8; ++i) if (block.write(&block, VFS_META_LBA + i, all + i * 512U) != 0) return -1;
...
int vfs_init(void) {
    ...
    int loaded = 0;
    zero(records, sizeof(records)); zero(allocation, sizeof(allocation)); generation = 0;
    disk_backed = ata_block_device_get(&block) == 0;
    records[0].used = 1; records[0].type = VFS_DIRECTORY; records[0].parent = -1;
    allocation[0] = 1;
    copy(records[0].name, "/", sizeof(records[0].name));
    if (disk_backed && block.read(&block, VFS_META_LBA, sector) == 0 &&
        *(uint32_t *)sector == VFS_MAGIC && *(uint32_t *)(sector + 4) == VFS_VERSION &&
        *(uint32_t *)(sector + 12) == VFS_FILE_MAX) {
        /* Metadata is intentionally bounded and fixed-size for crash-safe reads. */
        uint8_t all[4096]; uint32_t pos = 0;
        for (i = 0; i < 8; ++i) if (block.read(&block, VFS_META_LBA + i, all + i * 512U) != 0) break;
        if (i == 8) {
            for (i = 0; i < VFS_FILE_MAX; ++i) allocation[i] = all[16 + i];
            /* Reject corrupt allocation and directory references before mounting. */
            for (i = 0; i < VFS_FILE_MAX; ++i) {
                uint8_t *raw = (uint8_t *)&records[i]; uint32_t j;
                for (j = 0; j < sizeof(struct vfs_record); ++j) raw[j] = all[16 + VFS_FILE_MAX + pos++];
            }
            generation = *(uint32_t *)(all + 8);
            loaded = records[0].used && records[0].type == VFS_DIRECTORY &&
                     records[0].parent == -1 && allocation[0];
            for (i = 1; loaded && i < VFS_FILE_MAX; ++i)
                if (records[i].used != allocation[i] ||
                    (records[i].used && (records[i].parent < 0 || records[i].parent >= VFS_FILE_MAX ||
                    !records[records[i].parent].used || !records[i].name[0]))) loaded = 0;
        }
    }
...
static int create(const char *path, enum vfs_type type) {
    ...
    if (n >= 0 || parent_and_name(path, &parent, name) != 0 || !name[0]) {
        serial_printf("[MinOS VFS] create rejected path=%s existing=%d\n",
                      path ? path : "(null)", (uint64_t)n);
        return -1;
    }
    for (i = 1; i < VFS_FILE_MAX; ++i) if (!records[i].used) { slot = (int)i; break; }
    if (slot < 0) {
        serial_puts("[MinOS VFS] create rejected: no free records\n");
        return -1;
    }
    if (disk_backed && VFS_DATA_LBA + (uint32_t)slot * 8U + 8U > block.block_count) {
        serial_puts("[MinOS VFS] create rejected: data range unavailable\n");
        return -1;
    }
    ...
    allocation[slot] = 1;
    serial_printf("[MinOS VFS] created slot=%u name=%s parent=%d\n",
                  (uint64_t)slot, records[slot].name, (uint64_t)parent);
    return persist();
}
...
int vfs_rmdir(const char *p) { int n = find(p); uint32_t i; if (n <= 0 || records[n].type != VFS_DIRECTORY) return -1; for (i=1;i<VFS_FILE_MAX;++i) if(records[i].used&&records[i].parent==n)return -1; records[n].used=0; allocation[n]=0; return persist(); }
int vfs_remove(const char *p) { int n=find(p); if(n<=0||records[n].type!=VFS_FILE)return -1; records[n].used=0; allocation[n]=0; return persist(); }
int vfs_read(const char *p, char *b, uint32_t cap, uint32_t *size) {
    ...
    if (disk_backed) for (i = 0; i < 8; ++i) {
        if (block.read(&block, records[n].data_lba + i, sector) != 0) {
...
int vfs_write(const char *p, const char *d, uint32_t size, int append) {
    ...
    if (disk_backed) {
        ...
        for (i = 0; i < 8; ++i) {
            ...
            if (block.write(&block, records[n].data_lba + i, sector) != 0) return -1;
        }
    }
    return persist();
}
...
int vfs_usage(struct vfs_usage *usage) {
    uint32_t i, used = 0;
    if (!usage) return -1;
    for (i = 1; i < VFS_FILE_MAX; ++i) if (allocation[i]) used += VFS_CONTENT_MAX;
    usage->total_bytes = (VFS_FILE_MAX - 1U) * VFS_CONTENT_MAX;
    usage->used_bytes = used;
    usage->free_bytes = usage->total_bytes - used;
    return 0;
}
