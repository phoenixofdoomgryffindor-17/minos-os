...
static int console_arg(char **argv, uint32_t argc, uint32_t n, char *out) {
    if (n >= argc || console_path(argv[n], out) != 0) {
        serial_printf("[MinOS Console] argument rejected index=%u argc=%u\n",
...
static uint32_t console_words(char *line, char **argv, uint32_t max) {
    ...
    while (*line && n < max) {
        ...
        argv[n++] = write;
        {
            ...
            while (*line) {
                if (quote) {
                    if (*line == quote) quote = 0;
                    else *write++ = *line;
                } else if (*line == '\'' || *line == '"') quote = *line;
                else if (*line == ' ' || *line == '\t') break;
                else if (*line == '\\' && line[1]) *write++ = *++line;
                else *write++ = *line;
                ++line;
            }
        }
...
static void console_command(void) {
    ...
    if (argc == 0) {
    ...
    } else if (console_streq(argv[0], "uname")) {
        console_puts("MinOS x86_64 kernel\n");
        ...
        struct vfs_usage usage;
        if (vfs_usage(&usage) == 0) {
            console_puts("Filesystem  total  used  free\nMinOSFS     ");
    ...
    while (input_pop(&event) == 0) {
        ...
        if (event.key == INPUT_KEY_ENTER) {
            ...
            if (command_cursor < command_length) {
                ...
                --command_length; console_redraw_command();
            }
