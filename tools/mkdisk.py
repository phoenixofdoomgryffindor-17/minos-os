...
PRESERVE_OFFSET = 2048 * SECTOR_SIZE
RESERVED_SECTORS = 4
NUM_FATS = 2
ROOT_ENTRIES = 512
...
def create_fat16_image(efi_file_path, output_img_path):
    ...
    # Assemble the boot area separately so rebuilding the kernel does not erase
    # the MinOS filesystem stored after the boot files.
    existing_tail = None
    if os.path.exists(output_img_path):
        existing_size = os.path.getsize(output_img_path)
        if existing_size >= PRESERVE_OFFSET:
            with open(output_img_path, 'rb') as old_img:
                old_img.seek(PRESERVE_OFFSET)
                existing_tail = old_img.read()

    # Assemble Disk Image
    os.makedirs(os.path.dirname(output_img_path), exist_ok=True)
    temp_img_path = output_img_path + ".new"
    ...
    if existing_tail is not None:
        with open(temp_img_path, 'r+b') as img:
            img.seek(PRESERVE_OFFSET)
            img.write(existing_tail[:TOTAL_SECTORS * SECTOR_SIZE - PRESERVE_OFFSET])
            remaining = (TOTAL_SECTORS * SECTOR_SIZE - PRESERVE_OFFSET) - len(existing_tail)
            if remaining > 0:
                img.write(b'\x00' * remaining)
    os.replace(temp_img_path, output_img_path)
